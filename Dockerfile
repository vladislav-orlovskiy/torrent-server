...
RUN mkdir /opt/torrent-server
RUN apt update && apt install python3 python3-pip wget curl -y
COPY . /opt/torrent-server
WORKDIR /opt/server
RUN python3 -m pip install --upgrade pip
RUN python3 -m pip install -r /opt/torrent-server/requirements.txt
ENTRYPOINT [ "python3" "main.py"]

FROM prod as vagrant
